...
services:
  teal:
    ...
    networks:
      - platform_proxy
    # Platform-UI routing for Traefik is declared via a file in the
    # dynamic dir (written below as _platform.yml), NOT via container
    # labels. Traefik's docker provider is intentionally not used —
    # its bundled client speaks API v1.24 and modern docker daemons
    # reject the call.
...
# --------- Platform-UI route (file provider) ----------
# Traefik picks this up automatically (file provider has watch:true).
# We rewrite it on every install so the BASE_DOMAIN value is always
# current. The teal-platform-secure router is included unconditionally;
# Traefik silently ignores routers whose certresolver doesn't exist
# yet, so HTTPS turns on as soon as the admin saves an ACME email and
# Traefik is restarted — no separate file edit needed.
PLATFORM_ROUTE_FILE="$DATA_DIR/traefik/dynamic/_platform.yml"
log "Writing $PLATFORM_ROUTE_FILE"
cat > "$PLATFORM_ROUTE_FILE" <<ROUTE
# Generated by Teal install.sh — routes srv.sariakos.com to the
# platform UI. Per-app routes are managed by the engine in this same
# directory; their filenames are app slugs and don't collide with
# this file.
http:
  routers:
    teal-platform:
